...
parfor contCART=1:100
    for contTUM=0:32
        ...
        for iter=1:N % Calculamos 20 iteraciones por cada caso
            [vector_tum_final(iter),vector_CART] = fun_auto_bloque(semilla(iter),ntum0,contCART);
             fprintf('contCART: %d, contTUM: %d, iter: %d\n',contCART,contTUM,iter) % vemos por dónde va la simulación
        end
...
xlim([0 105])
ylim([500 1500])
legend('El tumor crece','El tumor se ha reducido menos de un 25%', 'El tumor se ha reducido entre un 25-50%','El tumor se ha reducido entre un 50-75%','El tumor se ha reducido más de un 75%','El tumor desaparece')
hold off
