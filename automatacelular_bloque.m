...
m_duplicar = zeros(n+1,n+1); % guardamos qué CAR-T están en fase de duplicación
...
rng('default');
rng(1000); % el número es la semilla
...
while k < ntum
    ...
    A(fila, col+1) = 2;
    A(fila+1, col+1) = 2;
    [A(1:n,1:n),B(1:n,1:n),m_vida(1:n,1:n),m_duplicar(1:n,1:n),m_contador(1:n,1:n),fila1,col1] = proliferar(fila1,col1,A(1:n,1:n),B(1:n,1:n),m_vida(1:n,1:n),m_duplicar(1:n,1:n),m_contador(1:n,1:n),0.7);
    k = (numel(A(A==2)))/4; % Contar nº de células que llevamos insertadas
end
...
while kT < nT
    ...
    if A(filaT,colT) == 0 % si el valor de la posicion es 0, añadimos la celula CAR-T
        A(filaT, colT) = 1;
        B(filaT,colT) = 9;
...
% Esto añade una fila de ceros al principio
A = [zeros(1,n+1); A];
% Esto añade una columna de ceros al principio
A = [zeros(n+2,1) A];
% Esto añade una fila de ceros al principio
B = [zeros(1,n+1); B];
% Esto añade una columna de ceros al principio
...
% Graficamos los movimientos de las células CAR-T, disparar, matar y proliferar células tumorales
T =2160; % tiempo en horas
for tiempo = 1:T
      ...
      % Hemos añadido m_duplicar a disparar. Aquí sabemos que células
      ...
      % y B ampliadas y a l función mover le tenemos que pasar matrices nxn.
      ...
      % Primero vemos que valores son igual a 1. Estas células son las que se van a duplicar
      ...
      % Creamos nuevos vectores para dibujar líneas horizontales y verticales en el punto de pico máximo.
