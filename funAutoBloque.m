function [f1,num_cell_t_new] = fun_auto_bloque(semilla,tum_inicial,carT_inicial)
    n = 200; % nxn tamaño de la matriz
    ntum = tum_inicial; %nº de celulas infectadas
    ...
    while kT < nT
        ...
        if A(filaT,colT) == 0 % Si el valor de la posición es 0, añadimos la célula CAR-T
            A(filaT, colT) = 1;
            B(filaT,colT) = 9;
    ...
    % Esto me añade una columna de ceros al principio
    B = [zeros(n+2,1) B];
    % Añadimos el borde en el resto de matrices
    ...
    % Graficamos los movimientos de las células T, disparar, matar  y proliferar células tumorales
...
% % % %           title(['hora ' num2str(tiempo) ' de  ' num2str(T)])
          ...
          % Actualizamos el valor de carga citotóxica que tiene cada célula
          % tumoral.
          % Añadimos m_duplicar a disparar. Aquí sabemos que células
...
f1 = length(carga_toxica_ctumoral);
